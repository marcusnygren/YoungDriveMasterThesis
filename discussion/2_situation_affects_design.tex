...
\section{How is the Design Affected by the Contextual Constraints?} % such as X, Y, Z, \Å?
  ...
  In the project, working with service design in another culture has been one of the hardest and most exciting challenges. When the interactions for iteration 1 were cancelled the day before the trip because of local elections, experts could empower and affirm that this was not out of the ordinary. A learning was that it is only possible to plan to a certain extent, but then changing the plan at the last second, is needed more than when one's own culture. This support from experts has been very helpful, having several people familiar with working in a different cultural context before, which is one of the reasons \cite{stickdorn} suggests making a project team multi-disciplinary. %, for example Grameen Foundation and the other Ugandian organizations mentioned in section \ref{sec:collaborators}.
